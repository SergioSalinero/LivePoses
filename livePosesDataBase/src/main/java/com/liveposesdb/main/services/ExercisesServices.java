...
public class ExercisesServices {
	...
	public boolean setPublicRoutine(PublicRoutine publicRoutine) {
        ...
        try {
			...
			String query = "INSERT INTO public_routines (exercises, breakTime, description, category) VALUES ('" + exercisesJson + "', '" + publicRoutine.getBreakTime() + "', '" + publicRoutine.getDescription() + "', '" +publicRoutine.getCategory() + "');";
	...
	public List<PublicRoutine> getCategoryRoutines(String category) {
		...
		for(String[] result : results) {
			try {
				List<CurrentRoutineExercises> exercises = mapper.readValue(result[1], new TypeReference<List<CurrentRoutineExercises>>() {});
				
				PublicRoutine pr = new PublicRoutine(exercises, Integer.parseInt(result[2]), result[3], result[4]);
