...
public class ExercisesServices {
	...
	public Exercise getStartSignal() {
		try {
            ResponseEntity<Exercise> response = restTemplate.getForEntity(RemoteServices.GET_START_SIGNAL, Exercise.class);
	...
	public List<CategoryCount> getCategoryCount() {
		try {
			ResponseEntity<CategoryCount[]> response = restTemplate.getForEntity(RemoteServices.GET_CATEGORY_COUNT, CategoryCount[].class);
